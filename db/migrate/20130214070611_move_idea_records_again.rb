class MoveIdeaRecordsAgain < ActiveRecord::Migration
  def up
    puts "***************************************"
    puts "***************************************"
    ...
    # truncate and reset id auto increment
    connection.execute("truncate idea_categories")
    connection.execute("truncate idea_inappropriate_reports")
    connection.execute("truncate idea_progresses")
    connection.execute("truncate ideas")
    ...
    old_ideas.each do |old_idea|
      i = Idea.create(
          ...
          :overall_votes => old_idea[4].nil? || old_idea[4] < 0 ? 0 : old_idea[4],
