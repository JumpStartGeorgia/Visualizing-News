  ...
  def edit
    ...
    @visualization = Visualization.find_by_permalink(params[:id])
		...
		if !locales_to_crop.empty?
			...
			to_crop = @visualization.visualization_translations.select{|x| x.locale == @locale_to_crop}.first
    ...
    @visualization = Visualization.new(params[:visualization])
		...
		if @visualization.visualization_type_id == Visualization::TYPES[:interactive]
			...
			@visualization.visualization_translations.each do |trans|
				...
				if @visualization.languages_internal.index(trans.locale) && trans.valid? && !trans.interactive_url.blank? && trans.visual_file_name.blank?
					...
					trans.visual = files[trans.locale]
				end
			end
		end
