class Admin::StoryTypesController < ApplicationController
  ...
  def new
    ...
    I18n.available_locales.each do |locale|
			@story_type.story_type_translations.build(:locale => locale)
		end
  ...
  def create
    ...
    respond_to do |format|
      if @story_type.save
        format.html { redirect_to admin_story_type_path(@story_type), notice: t('app.msgs.success_created', :obj => t('activerecord.models.story_type')) }
  ...
  def update
    ...
    respond_to do |format|
      if @story_type.update_attributes(params[:story_type])
        format.html { redirect_to admin_story_type_path(@story_type), notice: t('app.msgs.success_updated', :obj => t('activerecord.models.story_type')) }
        format.json { head :ok }
      else
